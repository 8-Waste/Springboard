...
def show_images(path, feat, pcts):
    ...
    for pct in pcts:
        ...
        for i in range(len(path)):
            if feat == None:
                img = cv2.imread(path[i])  # read image in BGR
                img = get_crop(img, pct)
                img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
            else:
                img = cv2.imread(path[i], 0)  # read gray scale image
                img = get_crop(img, pct)
...
def save_faces(img):
    import os
    import cv2
    img2 = img.replace(r'\sharp', r'\faces')
...
def convert_files(source_path, target_path, file):
    import os
    import rawpy
    import imageio
    import shutil
...
def move_classified_results(soccer_images_results):
    import os
    import shutil
    img, res = soccer_images_results
    if res == 0:  # image is blurred
        img2 = img.replace(r'\tiff', r'\blurry')
        if not os.path.isfile(img2):
            shutil.copy(img, img2)
    elif res == 1:  # image is sharp
        img2 = img.replace(r'\tiff', r'\sharp')
        if not os.path.isfile(img2):
            shutil.copy(img, img2)
    return True
